  ...
  useEffect(() => {
    async function load() {
      ...
      const { data, error: e } = await supabase
        .from('offers').select('*').order('created_at', { ascending: false }).limit(6);
  ...
  return (
    <div className="page px-4 py-5 space-y-6">
      <div className="bg-gradient-to-br from-primary to-accent rounded-3xl p-5 text-white relative overflow-hidden">
        ...
        <p className="text-white/70 text-xs font-semibold uppercase tracking-widest mb-1">{t('home.welcome')}</p>
        <h1 className="font-display font-extrabold text-2xl leading-tight">{t('app.name')} 🩺</h1>
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          { label: t('home.offers'),  value: stats.total, icon: TrendingUp, color: 'bg-blue-50 text-blue-600' },
          { label: t('home.average'), value: stats.avg > 0 ? `${Math.round(stats.avg/1000)}k€` : '—', icon: Shield, color: 'bg-emerald-50 text-emerald-600' },
          { label: t('home.centers'), value: stats.centers, icon: Search, color: 'bg-violet-50 text-violet-600' },
      ...
      <section>
        <div className="flex items-center justify-between mb-3">
          <h2 className="section-title">{t('home.latestOffers')}</h2>
        ...
        <div className="space-y-3">
          {loading ? (
            ...
            <div className="card p-6 text-center">
              <p className="text-slate-400 text-sm">{t('home.noOffers')}</p>
            </div>
          ) : (
      ...
      <div className="card p-4 bg-gradient-to-r from-slate-50 to-blue-50 flex items-center gap-3">
        ...
        <div>
          <p className="font-semibold text-slate-900 text-sm">{t('home.secureTitle')}</p>
          <p className="text-xs text-slate-500">{t('home.secureText')}</p>
        </div>
      </div>
    </div>
  );
