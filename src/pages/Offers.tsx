    ...
    <div className="page">
      <div className="sticky top-14 z-30 bg-surface/95 backdrop-blur-md px-4 pt-3 pb-2 border-b border-slate-100">
        <div className="relative">
          ...
          <input className="input pl-9 pr-9" placeholder={t('offers.placeholder')} value={query} onChange={e => setQuery(e.target.value)} />
        ...
        <p className="text-xs text-slate-400 mt-2">{filtered.length} {filtered.length !== 1 ? t('nav.offers').toLowerCase() : t('nav.offers').toLowerCase().replace(/s$/, '')}</p>
      </div>
